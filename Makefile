...
build/lib/libluajit-5.1.a: build/$(LUAJIT_DIR)
	(cd $(LUAJIT_DIR); make install PREFIX=$(realpath build) CC=$(CC) STATIC_CC="$(CC) -fPIC" CCDEBUG="-g" XCFLAGS="-DLUAJIT_ENABLE_GC64 -DLUA_USE_ASSERT") # -DLUAJIT_USE_SYSMALLOC -DLUAJIT_USE_VALGRIND
