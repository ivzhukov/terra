...
int terra_loadbytecodes(lua_State *L, const unsigned char *bytecodes, size_t size,
                        const char *name) {
...
int terra_registerinternalizedfiles(lua_State *L, int terratable) {
    ...
    for (int i = 0; luafile_indices[i] != -1; i++) {
        ...
        int err = terra_loadbytecodes(L, headerfile_contents[idx], headerfile_sizes[idx],
                                      name.substr(1, name.size() - 4).c_str());
        if (err) return err;
    }
#else
    lua_getglobal(L, "package");
